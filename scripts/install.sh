...
BIN_DIR="${AGENTATTENTION_INSTALL_BIN_DIR:-$HOME/.local/bin}"
STATE_DIR="${AGENTATTENTION_INSTALL_STATE_DIR:-$HOME/.local/state/agentattention}"
TARGET="$BIN_DIR/agentattention"
RECEIPT="$STATE_DIR/deployed-sha"
SERVER_CONFIG="${AGENTATTENTION_INSTALL_SERVER_CONFIG:-${AGENTATTENTION_CONFIG:-$HOME/.config/agentattention/config.json}}"
CLIENT_CONFIG="${AGENTATTENTION_INSTALL_CLIENT_CONFIG:-${AGENTATTENTION_CLIENT_CONFIG:-$HOME/.config/agentattention/client.json}}"
UPSTREAM_ORIGIN="https://github.com/possibilities/agentattention.git"
EXPECTED_ORIGIN="${AGENTATTENTION_INSTALL_EXPECTED_ORIGIN:-$UPSTREAM_ORIGIN}"
TMP_PATH=""
...
With no option, installs agentattention. Installation runs Bun's frozen
dependency install, links ~/.local/bin/agentattention to this checkout, writes
the deployed Git SHA, and initializes the default server and local-client
configuration when the server configuration is absent.

Set AGENTATTENTION_INSTALL_BIN_DIR, AGENTATTENTION_INSTALL_STATE_DIR,
AGENTATTENTION_INSTALL_SERVER_CONFIG, and AGENTATTENTION_INSTALL_CLIENT_CONFIG
to use other locations, including for hermetic tests.
USAGE
}

die() {
  printf 'agentattention installer: %s\n' "$1" >&2
  exit "${2:-1}"
}
...
normalized_origin() {
  ...
  origin="${origin%/}"
  case "$origin" in
    https://github.com/possibilities/agentattention|https://github.com/possibilities/agentattention.git|git@github.com:possibilities/agentattention|git@github.com:possibilities/agentattention.git|ssh://git@github.com/possibilities/agentattention|ssh://git@github.com/possibilities/agentattention.git)
      printf '%s\n' "$UPSTREAM_ORIGIN"
      ;;
    *)
      printf '%s\n' "$origin"
...
validate_managed_checkout() {
  ...
  validate_safe_file "$source" "agentattention source command"
  [[ -x "$source" ]] || die "refusing non-executable agentattention source command: $source"
  sha="$(checkout_head "$root")" || die "refusing agentattention source outside an exact Git checkout: $root"
  origin="$(git -C "$root" remote get-url origin 2>/dev/null)" || die "refusing agentattention source without an origin: $root"
  [[ "$(normalized_origin "$origin")" == "$EXPECTED_ORIGIN" ]] || die "refusing agentattention source with foreign origin: $root"
  MANAGED_ROOT="$root"
  MANAGED_SHA="$sha"
}
...
bootstrap_configuration() {
  validate_path "$SERVER_CONFIG" "server configuration"
  ...
  if [[ -e "$CLIENT_CONFIG" || -L "$CLIENT_CONFIG" ]]; then
    die "refusing to create a new server configuration beside an existing unmatched client configuration: $CLIENT_CONFIG"
  fi
  AGENTATTENTION_CONFIG="$SERVER_CONFIG" \
    AGENTATTENTION_CLIENT_CONFIG="$CLIENT_CONFIG" \
    "$TARGET" --json init
}

install_agentattention() {
  local sha
  ...
  TMP_PATH="$BIN_DIR/.agentattention-link.$$.$RANDOM"
...
uninstall_agentattention() {
  ...
  if (( removed )); then
    note "removed command and deployment receipt; configuration and database were preserved"
  else
    note "agentattention is not installed"
  fi
}

if (( $# > 1 )); then die "expected at most one installer option" 2; fi
case "${1:---install}" in
  --install) install_agentattention ;;
  --uninstall) uninstall_agentattention ;;
  --help|-h) usage ;;
